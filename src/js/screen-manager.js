...
$(document).ready(function () {
  ...
  $('.lang__en').on('click', function () {
    ...
    $('.annotation-screen').addClass('active')
    setTimeout(function () {
      $('.annotation-screen').addClass('visible')
    }, 500)
  })
  ...
  $('.annotation-screen__action-button').on('click', function () {
    ...
    $('.annotation-screen').removeClass('visible')
    setTimeout(function () {
      $('.annotation-screen').removeClass('active')
    }, 500)
    // Показываем меню
    ...
    if (RegimeLanguage) {
      ...
      // Если дрейф не кончился, то показываем следующую команду
        ...
        switch (method) {
          ...
          case 0:
            $('.forward')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>FORWARD')
              .addClass('visible')
            break
          ...
          case 1:
            $('.left')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>LEFT')
              .addClass('visible')
            break
          ...
          case 2:
            $('.right')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>RIGHT')
              .addClass('visible')
            break
          ...
          case 3:
            ...
            // Показываем команду
            $('.stop')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>STOP<br>' + timer)
              .addClass('visible')
          ...
          case 4:
            ...
            $('.transport')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>GET ON A PUBLIC TRANSPORT THAT CONTAINS A NUMBER OF ROUTE ' + transport_number + ' AND GO ' + transport_stop_value + ' STOPS')
              .addClass('visible')
            break
          ...
          case 8:
            $('.east')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>GO TO THE EAST ' + value_of_acts + ' QUARTALS')
              .addClass('visible')
            break
          ...
          case 9:
            ...
            $('.street')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>FIND THE NEAREST STREET THAT NAME BEGINS WITH A LETTER ' + '"' + letter_eng + '"')
              .addClass('visible')
            break
        }
      }
    ...
    //На русском
      ...
      if (CurrentMethodValue == value_of_methods) {
        ...
        switch (method) {
          case 0:
            $('.forward')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>ПРЯМО')
              .addClass('visible')
            break

          case 1:
            $('.left')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>ВЛЕВО')
              .addClass('visible')
            break

          case 2:
            $('.right')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>ВПРАВО')
              .addClass('visible')
            break
          ...
          case 3:
            var timer = 60
            $('.stop')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>СТОП<br>' + timer)
              .addClass('visible')
          ...
          case 4:
            ...
            $('.transport')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>СЯДЬ НА ОБЩЕСТВЕННЫЙ ТРАНСПОРТ, НОМЕР КОТОРОГО СОДЕРЖИТ ЦИФРУ ' + transport_number + ' И ПРОЕДЬ ' + transport_stop_value + ' ОСТАНОВОК')
              .addClass('visible')
            break
          ...
          case 8:
            $('.east')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>ИДИ НА ВОСТОК ' + value_of_acts + ' КВАРТАЛА')
              .addClass('visible')
            break
          ...
          case 9:
            ...
            $('.street')
              .html(CurrentMethodValue + ' / ' + value_of_methods + '<br><br>НАЙДИ БЛИЖАЙШУЮ УЛИЦУ, НАЗВАНИЕ КОТОРОЙ НАЧИНАЕТСЯ НА БУКВУ ' + '"' + letter + '"')
              .addClass('visible')
            break
        }
      }
...
function mapDerive() {
  ...
  document.querySelector('.detailedMap').addEventListener('click', function () {
    clickButton(0)
  })
  document.querySelector('.normalMap').addEventListener('click', function () {
    clickButton(1)
  })
  document.querySelector('.minimalMap').addEventListener('click', function () {
    clickButton(2)
  })
  ...
  function onLocationFound(e) {
    ...
    if (distance < 100) {
      ...
      toMarker = new L.Marker([toMarkerLat, toMarkerLng], {
        icon: toIcon,
      }).addTo(map) // Генерим новую на основе новых координат
      ...
      let bounds = L.latLngBounds([
        [e.latlng.lat + 0.008, e.latlng.lng + 0.008],
        [toMarkerLat - 0.008, toMarkerLng - 0.008],
      ])
  ...
  let osmMap = L.tileLayer('https://{s}.tile.osm.org/{z}/{x}/{y}.png', {
    detectRetina: true,
  })
  let stamenToner = L.tileLayer('https://tiles.stadiamaps.com/tiles/stamen_toner/{z}/{x}/{y}{r}.png', { detectRetina: true })
  let stamenTonerLabels = L.tileLayer('https://tiles.stadiamaps.com/tiles/stamen_toner-labels/{z}/{x}/{y}{r}.png', { detectRetina: true })
...
function mapDeriveRu() {
  ...
  document.querySelectorAll('.detailedMap')[1].addEventListener('click', function () {
    clickButton(0)
  })
  document.querySelectorAll('.normalMap')[1].addEventListener('click', function () {
    clickButton(1)
  })
  document.querySelectorAll('.minimalMap')[1].addEventListener('click', function () {
    clickButton(2)
  })
  document.querySelectorAll('.selectButton')[1].addEventListener('click', function () {
    selectButton()
  })
